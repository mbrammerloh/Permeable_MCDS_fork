...
SphereMap::SphereMap()
    : x0{.0,.0,.0},
      entries_(static_cast<size_t>(N) * N * N),
      voxel_size_{1.0, 1.0, 1.0},
      step_size_{1.0/N, 1.0/N, 1.0/N}
      {}

SphereMap::SphereMap(const SphereMap &spm){
   x0 = spm.x0; 
}
...
bool SphereMap::add_sphere_to_entry(int x, int y, int z, const SphereIds& ids, bool no_duplicates) {
    ...
    if (no_duplicates) {
        if (find(v.begin(), v.end(), ids) != v.end()) return false;
    }
    v.push_back(ids);
    return true;
}
...
const vector<SphereIds>& SphereMap::spheres(int x, int y, int z) const {
    return entry_spheres(x, y, z);
}

vector<SphereIds>& SphereMap::spheres(int x, int y, int z) {
    return entry_spheres(x, y, z);
}

size_t SphereMap::lin(int x, int y, int z) {
    assert(0 <= x && x < N);
    assert(0 <= y && y < N);
    assert(0 <= z && z < N);
    return static_cast<size_t>(x)
         + static_cast<size_t>(N) * (static_cast<size_t>(y)
                                          + static_cast<size_t>(N) * static_cast<size_t>(z));
}

const vector<SphereIds>& SphereMap::entry_spheres(int x, int y, int z) const {
    return entries_[lin(x, y, z)].spheres;
}

vector<SphereIds>& SphereMap::entry_spheres(int x, int y, int z) {
    return entries_[lin(x, y, z)].spheres;
}
...
bool SphereMap::add_sphere(Sphere& sphere,  SphereIds& ids){
    ...
    vector<int> number_of_entries = get_number_of_entries(sphere);

    const vector<int> index_center = get_index_of_point(sphere.center);
...
vector<int> SphereMap::get_number_of_entries(Sphere & sphere){
    vector<int> number_of_entries;
    for (int i = 0; i < 3; i++){
        number_of_entries.push_back(int(round(sphere.radius / step_size_[i]))+1); // the number of entries to test in each direction from the sphere center, +1 for safety / limiting cases
    }
    return number_of_entries;
}

vector<SphereIds> SphereMap::get_potentially_overlapping_spheres_ids(Sphere & sphere){
    vector<SphereIds> sphere_ids;
            ...
            for (   int k = index_center[2]-number_of_entries[2]; k <= index_center[2]+number_of_entries[2]; k++){  
                if (i>=0 && i < N && j>=0 && j < N && k>=0 && k < N){
                    ...
                    for (uint l = 0; l < these_sphere_ids.size(); l++){
                        ...
                        if (!(find(sphere_ids.begin(), sphere_ids.end(), these_sphere_ids[l]) != sphere_ids.end())) {
...
bool SphereMap::is_sphere_in_map_voxel(vector<int> & indices, Eigen::Vector3d& sphere_center, double & radius)
{
    Eigen::Vector3d cube_corner0;
    Eigen::Vector3d cube_corner1;

    for (int i = 0; i < 3; i++){
        cube_corner0[i] = indices[i] * step_size_[i] + x0[i];
        cube_corner1[i] = (indices[i]+1) * step_size_[i] + x0[i];
    }
...
vector<int> SphereMap::get_index_of_point(Eigen::Vector3d & point)
{
    vector<int> indices;
    for (int i = 0; i < 3; i++){
        indices.push_back((point[i]-x0[i]) / step_size_[i]);
    }
    return indices;
}
